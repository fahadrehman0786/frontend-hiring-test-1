...
export default function EnhancedTable({ rows1, token }) {
  ...
  const handleStatusClick = (event, row) => {
    // Perform the desired action when the status is clicked
    console.log("Status clicked for row:", row);
  };
  
  const handleArchiveCall = async (row) => {
    try {
      ...
      const updatedCall = { ...row, is_archived: true };

      // Find the index of the call in the data array
      const callIndex = rows.findIndex((c) => c.id === row.id);
        ...
        <TableContainer>
          ...
          >
            <EnhancedTableHead
              ...
              onStatusClick={handleArchiveCall} // Pass the handleStatusClick function
            />
            <TableBody>
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell
                      align="left"
                      onClick={(event) => handleArchiveCall(event, row)} // Handle the click event on status
